<?php
	...
	@$a=mysql_query("DELETE FROM manual_classes");
	$role = 3;
    @$b=mysql_query("ALTER TABLE manual_classes AUTO_INCREMENT = 1");
	foreach($values as $v)
	{
		//dien so tuan
		$lv_name=$v[0];
		...
		//chen vao db
		@$a=mysql_query("INSERT INTO manual_classes (`id`, `lv_name`, `cl_name`, `times`, `days`, `startday`, `weeks`, `midterm`, `endday`, `tutors`, `recieveday`) VALUES (NULL, '{$v[0]}', '{$v[1]}', '{$v[2]}', '{$v[3]}', '{$v[4]}', '{$weeks}', '{$mid}', '{$checkout}', '{$v[8]}', '{$v[9]}')");			
	}
